-- ============================================================
-- 20260616_06_bloqueo_por_partido.sql
-- ------------------------------------------------------------
-- BLOQUEO POR PARTIDO INDIVIDUAL
--
-- Antes: la apuesta entera se cerraba con `fecha_cierre` (una sola
-- fecha límite global). Si una apuesta agrupaba muchos partidos,
-- todos quedaban bloqueados desde el primer partido.
--
-- Ahora: `fecha_cierre` sigue siendo el CIERRE GLOBAL (la apuesta
-- queda abierta hasta el final de la fase), y ADEMÁS cada partido se
-- bloquea individualmente a su hora de inicio:
--     bloqueado  <=>  fecha_hora <= now()
--                     OR estado IN ('en_vivo','finalizado','cancelado')
--
-- Se actualizan los 4 puntos de validación del servidor:
--   1) RPC  guardar_predicciones_apuesta   (guardado en lote)
--   2) Trigger validar_prediccion_integridad (insert/update fila a fila)
--   3) Política RLS predicciones_insert
--   4) Política RLS predicciones_update
--
-- Rollback disponible en: 20260616_06_bloqueo_por_partido_ROLLBACK.sql
-- ============================================================

-- ════════════════════════════════════════════════════════════
-- 1) RPC: guardar_predicciones_apuesta
--    Saltea (sin guardar) los partidos que ya comenzaron, sin
--    afectar el guardado de los demás ni pisar predicciones previas.
-- ════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- ═══ 3. VALIDAR APUESTA (cierre global) ═══
  ...
  -- ═══ 5. VALIDAR PARTIDOS (pertenencia) ═══
  ...
  LOOP
    ...
    -- ═══ BLOQUEO POR PARTIDO ═══
    -- Si el partido ya empezó (por hora de inicio o por estado) se saltea:
    -- no se inserta ni se pisa la predicción previa.
    SELECT fecha_hora, estado INTO v_fh, v_estado_p
    FROM partidos WHERE id = v_partido_id;

    IF v_fh IS NULL
       OR v_fh <= now()
       OR v_estado_p IN ('en_vivo', 'finalizado', 'cancelado') THEN
  ...
  UPDATE apuestas SET total_participantes = (
    CASE WHEN v_apuesta.tipo = 'grupos'
      THEN (SELECT count(DISTINCT area_id) FROM predicciones WHERE apuesta_id = p_apuesta_id AND area_id IS NOT NULL)
      ELSE (SELECT count(DISTINCT user_id) FROM predicciones WHERE apuesta_id = p_apuesta_id)
    END
...
-- ════════════════════════════════════════════════════════════
-- 2) TRIGGER: validar_prediccion_integridad
--    Rechaza una predicción si su partido ya comenzó.
--    Mantiene el bypass de scoring (UPDATE que solo cambia puntos).
-- ════════════════════════════════════════════════════════════
...
begin
  ...
  -- Ej: UPDATE predicciones SET puntos = ... (no cambia la predicción en sí)
  if tg_op = 'UPDATE'
  ...
  -- A + B: apuesta abierta y vigente (cierre global)
  ...
  -- D2: BLOQUEO POR PARTIDO — no se admite predecir un partido ya iniciado
  select fecha_hora, estado into v_fh, v_estado_p
  from public.partidos where id = new.partido_id;
  if v_fh is null
     or v_fh <= now()
     or v_estado_p in ('en_vivo', 'finalizado', 'cancelado') then
    raise exception 'El partido % ya comenzó: no se admiten ni se modifican predicciones', new.partido_id;
  end if;
  ...
  if public.es_fase_eliminatoria(v_fase) and new.pred_local = new.pred_visitante then
    ...
    if not exists (
      ...
      where id = new.partido_id and (local = new.pred_clasificado or visitante = new.pred_clasificado)
    ) then
...
-- ════════════════════════════════════════════════════════════
-- 3) RLS: predicciones_insert
--    Agrega la condición "el partido no empezó".
-- ════════════════════════════════════════════════════════════
...
CREATE POLICY predicciones_insert ON public.predicciones
  ...
  WITH CHECK (
    ...
    AND (
      (EXISTS (
        SELECT 1 FROM apuestas a
        WHERE a.id = predicciones.apuesta_id AND a.tipo = 'libre'
      ))
      OR (
        ...
        AND (EXISTS (
          SELECT 1 FROM apuestas a
          WHERE a.id = predicciones.apuesta_id AND a.tipo = 'grupos'
        ))
      )
    )
  );
