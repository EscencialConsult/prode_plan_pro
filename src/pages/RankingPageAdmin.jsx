...
.rk-detail-btn-podio {
  background:rgba(31,31,35,.06);color:#1f1f23;
}
.rk-detail-btn-podio:hover:not(:disabled) { background:rgba(31,31,35,.12) }
.rk-detail-btn-podio.active { background:#1f1f23;color:#c02727 }

.rk-detail-btn-mini {
  background:rgba(31,31,35,.05);color:#6e6f73;
  font-size:8px;padding:4px 8px;
}
.rk-detail-btn-mini:hover:not(:disabled) { background:rgba(31,31,35,.1);color:#1f1f23 }
.rk-detail-btn-mini.active { background:#1f1f23;color:#c02727 }
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '2rem 1.5rem 3rem' }}>
        ...
        <div className="rk-in" style={{ marginBottom: '1.5rem' }}>
          <h1 style={{
            ...
            <span style={{ color: '#1f1f23' }}>RANKING </span>
            <span style={{ color: '#c02727' }}>ADMIN</span>
          </h1>
          <p style={{ fontSize: '.84rem', color: '#6e6f73', margin: 0 }}>
            {sel ? sel.titulo : 'Ranking global acumulado · Seleccioná una apuesta para ver el detalle individual'}
          </p>
        </div>

        <div className="rk-shell" style={{ display: 'flex', height: 'calc(100vh - 200px)', minHeight: 520, borderRadius: 20, overflow: 'hidden', boxShadow: '0 8px 48px rgba(31,31,35,.14)' }}>

          <div className="rk-sidebar">
            <div style={{ padding: '20px 16px 14px', borderBottom: '1px solid #e4e4e7' }}>
...
function Banner({ apuesta, meta, loading }) {
  return (
    <div style={{ borderRadius: 14, marginBottom: 24, background: 'linear-gradient(125deg,#1f1f23 0%,#2e2e33 100%)', padding: '18px 22px', position: 'relative', overflow: 'hidden' }}>
...
function PrediccionesPanel({ user, predicciones, apuesta, onClose }) {
  return (
    <div className="rk-in" style={{
      ...
      padding: '1rem 1.2rem',
      marginBottom: 20,
      boxShadow: '0 4px 20px rgba(31,31,35,.06)',
    }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '.9rem', paddingBottom: '.7rem', borderBottom: '1px solid #e4e4e7' }}>
        <div>
          <p style={{ fontSize: 9, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.22em', color: '#94a3b8', margin: '0 0 3px' }}>
            Predicciones de
          </p>
          <p style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 18, color: '#1f1f23', margin: 0, letterSpacing: '.02em', lineHeight: 1 }}>
            {user?.nombre}
          </p>
        </div>
        <button
          ...
          onMouseEnter={e => e.currentTarget.style.background = 'rgba(31,31,35,.12)'}
          onMouseLeave={e => e.currentTarget.style.background = 'rgba(31,31,35,.06)'}
        >
      ...
      {predicciones.length > 0 ? (
        ...
        <p style={{ fontSize: '.8rem', color: '#a9a9ae', margin: 0, textAlign: 'center', padding: '.8rem 0' }}>
...
function PrediccionRow({ pred, apuesta }) {
  ...
  if (puntosDlBackend !== null) {
    ...
    if (puntosDlBackend === ptsExacto) {
      ...
      borderC = '#c0272740'; bgC = '#c027270a'; badge = { c: '#c02727', label: `+${puntosDlBackend}` }
      ...
      if (tieneResultado) {
        ...
        borderC = '#e4e4e7'; bgC = '#fafafa'; badge = null
      }
    }
    ...
    else if (dif) { borderC = '#c0272740'; bgC = '#c027270a'; badge = { c: '#c02727', label: `+${ptsDif}` } }
  ...
  return (
    <div style={{
      ...
      <div style={{ minWidth: 0 }}>
        <p style={{ fontSize: '.78rem', fontWeight: 600, color: '#1f1f23', margin: '0 0 .12rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
      ...
      <div style={{ textAlign: 'center' }}>
        ...
        <p style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: '1.05rem', fontWeight: 700, color: '#1f1f23', margin: 0, letterSpacing: '.04em' }}>
      ...
      <div style={{ textAlign: 'center' }}>
        ...
        <p style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: '1.05rem', fontWeight: 700, color: tieneResultado ? '#1f1f23' : '#a9a9ae', margin: 0, letterSpacing: '.04em' }}>
  ...
  return (
    <div className="rk-in">
      ...
      <div style={{ borderRadius: 14, marginBottom: 24, background: 'linear-gradient(125deg,#1f1f23 0%,#2e2e33 100%)', padding: '18px 22px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, position: 'relative' }}>
          <div>
            <span style={{ fontSize: 9, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.22em', color: 'rgba(192,39,39,.55)', display: 'block', marginBottom: 4 }}>RANKING GLOBAL ACUMULADO</span>
    ...
    <div style={{ marginTop: 20 }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1 }}>
          ...
          <span style={{ fontSize: 8, fontWeight: 700, background: 'rgba(31,31,35,.05)', color: '#c8d0dc', padding: '1px 6px', borderRadius: 4 }}>+{otros.length}</span>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, paddingTop: 12 }}>
          {otros.map((u, idx) => (
            <div key={u.user_id} style={{
              ...
              onMouseEnter={e => { e.currentTarget.style.background = '#fafafa'; e.currentTarget.style.borderColor = '#e8e3db' }}
              onMouseLeave={e => { e.currentTarget.style.background = '#fff'; e.currentTarget.style.borderColor = '#f0f0f2' }}
            >
              <span style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 11, fontWeight: 700, color: '#94a3b8', textAlign: 'center' }}>#{idx + 4}</span>
              <div style={{ minWidth: 0 }}>
                <p style={{ fontWeight: 600, fontSize: 12, color: '#1f1f23', margin: '0 0 1px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{u.nombre}</p>
              ...
              <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-start' }}>
                {[{ v: u.aciertos_exactos, c: '#22c55e' }, { v: u.aciertos_diferencia || 0, c: '#c02727' }].map((x, i) => (
                  x.v > 0 && (
              ...
              <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 16, fontWeight: 700, color: '#1f1f23', textAlign: 'right' }}>{u.puntos_totales}</div>
            </div>
          ))}
        </div>
      )}
    ...
    <div style={{ marginTop: 20 }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1 }}>
          ...
          <span style={{ fontSize: 8, fontWeight: 700, background: 'rgba(31,31,35,.05)', color: '#c8d0dc', padding: '1px 6px', borderRadius: 4 }}>
            +{otros.length}
          </span>
          <div style={{ flex: 1, height: '1px', background: 'linear-gradient(90deg,#e8e3db,transparent)', marginLeft: 8 }} />
        </div>
      ...
      {exp && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, paddingTop: 12 }}>
          {otros.map((u, idx) => {
            ...
            return (
              <div key={u.user_id} style={{
                background: isMe ? 'rgba(192,39,39,.1)' : '#fff',
                border: isMe ? '1.5px solid #c02727' : '1px solid #f0f0f2',
                boxShadow: isMe ? '0 0 0 1px rgba(192,39,39,.3), 0 2px 8px rgba(192,39,39,.12)' : 'none',
                borderRadius: 10,
                overflow: 'hidden',
                transition: 'all .15s',
              }}>
                ...
                <div style={{
                  ...
                  <div style={{ minWidth: 0 }}>
                    <p style={{ fontWeight: isMe ? 700 : 500, fontSize: 11, color: isMe ? '#c02727' : '#1f1f23', margin: '0 0 1px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {u.nombre}
                      {isMe && (
                        <span style={{
                          fontSize: 8,
                  ...
                  <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 15, fontWeight: 700, color: '#1f1f23', textAlign: 'right' }}>{u.puntos_totales}</div>
                ...
                {isExpanded && (
                  <div style={{ borderTop: '1px solid #e4e4e7', padding: '.8rem 1rem', background: '#fafafa' }}>
                    {(predicciones[u.user_id] || []).length > 0 ? (
                      <PrediccionesGrid predicciones={predicciones[u.user_id]} apuesta={apuesta} />
                    ) : (
                      <p style={{ fontSize: '.7rem', color: '#a9a9ae', margin: 0, textAlign: 'center' }}>Sin predicciones</p>
                    )}
                  </div>
                )}
              </div>
