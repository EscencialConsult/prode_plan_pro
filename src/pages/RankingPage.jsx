    ...
    <AppShell>
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '2rem 1.5rem 3rem' }}>
        ...
        <div className="rk-in" style={{ marginBottom: '1.5rem' }}>
          <h1 style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 'clamp(2.4rem,6vw,3.5rem)', color: '#1f1f23', margin: '0 0 .3rem', lineHeight: 1, letterSpacing: '.02em' }}>
            RANKING ADMIN
          </h1>
        ...
        {!sel ? (
          ...
          <div style={{ display: 'grid', gap: '1rem' }}>
            ...
            {!lb && bets.length === 0 && (
              <div style={{ textAlign: 'center', padding: '3rem 1.5rem', background: '#fff', border: '1px solid #e4e4e7', borderRadius: 14 }}>
                <p style={{ fontWeight: 600, color: '#6e6f73', margin: 0 }}>Sin apuestas creadas</p>
              </div>
            )}
            ...
            {!lb && bets.map(bet => {
              ...
              return (
                <div key={bet.id}
                  ...
                  onMouseEnter={e => { e.currentTarget.style.borderColor = '#c02727'; e.currentTarget.style.boxShadow = '0 4px 16px rgba(192,39,39,.15)' }}
                  onMouseLeave={e => { e.currentTarget.style.borderColor = '#e4e4e7'; e.currentTarget.style.boxShadow = 'none' }}
                >
                  <div>
                    <h3 style={{ fontWeight: 700, fontSize: '1rem', color: '#1f1f23', margin: '0 0 .3rem' }}>{bet.titulo}</h3>
                    <p style={{ fontSize: '.75rem', color: '#6e6f73', margin: 0 }}>
                  ...
                  <div style={{ textAlign: 'center' }}>
                    ...
                    <span style={{
                      ...
                      padding: '.3rem .7rem',
                      borderRadius: 99,
                      background: activa ? 'rgba(34,197,94,.1)' : 'rgba(192,39,39,.1)',
                      color: activa ? '#22c55e' : '#9e1f1f',
                    }}>
                      {activa ? 'ACTIVA' : bet.estado === 'finalizada' ? 'FINALIZADA' : 'CERRADA'}
                    </span>
                  </div>
                  <div style={{ textAlign: 'right', color: '#6e6f73' }}>→</div>
                </div>
              )
            })}
          </div>
          ...
          <div className="rk-in">
            ...
            {loading ? (
              ...
              <div style={{ textAlign: 'center', padding: '3rem', background: '#fff', border: '1px solid #e4e4e7', borderRadius: 14 }}>
                <p style={{ fontWeight: 600, color: '#6e6f73' }}>Sin participantes todavía</p>
              </div>
            ) : (
              <div style={{ display: 'grid', gap: '1.2rem' }}>
                {tabla.map((u, idx) => (
                  <div key={u.user_id} style={{ background: '#fff', border: '1px solid #e4e4e7', borderRadius: 14, overflow: 'hidden' }}>
                    ...
                    >
                      <span style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: '1.1rem', fontWeight: 700, color: '#1f1f23' }}>{idx + 1}</span>

                      <div style={{ textAlign: 'left', minWidth: 0 }}>
                        <p style={{ fontWeight: 600, fontSize: '.95rem', color: '#1f1f23', margin: '0 0 .2rem' }}>{u.nombre}</p>
                        <p style={{ fontSize: '.75rem', color: '#6e6f73', margin: 0 }}>{u.predicciones} predicciones</p>
                      </div>
                      ...
                      <div style={{ textAlign: 'right', fontFamily: "'Bebas Neue',sans-serif", fontSize: '1.3rem', fontWeight: 700, color: '#1f1f23' }}>
                        {u.puntos_totales}
                      </div>
                    </button>
                    ...
                    {expandedUser === u.user_id && (
                      <div style={{ borderTop: '1px solid #e4e4e7', padding: '1rem 1.5rem', background: '#fafafa' }}>
                        <p style={{ fontSize: '.75rem', fontWeight: 700, textTransform: 'uppercase', color: '#6e6f73', margin: '0 0 1rem' }}>
                        ...
                        <div style={{ display: 'grid', gap: '.8rem' }}>
                          {predicciones[u.user_id]?.length ? (
                            predicciones[u.user_id].map((pred, i) => (
                              <div key={i} style={{
                                ...
                                <div style={{ minWidth: 0 }}>
                                  <p style={{ fontSize: '.85rem', fontWeight: 600, color: '#1f1f23', margin: '0 0 .2rem' }}>
                                    {pred.equipo_local} vs {pred.equipo_visitante}
                                  </p>
                                  <p style={{ fontSize: '.7rem', color: '#6e6f73', margin: 0 }}>
                                    Jornada {pred.jornada} · {pred.fecha_partido ? new Date(pred.fecha_partido).toLocaleDateString('es-AR') : '—'}
                                  </p>
                                </div>

                                <div style={{ textAlign: 'center' }}>
                                  <p style={{ fontSize: '.7rem', color: '#6e6f73', margin: '0 0 .2rem' }}>Predicción</p>
                                  <p style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: '1.2rem', fontWeight: 700, color: '#1f1f23', margin: 0 }}>
                                    {pred.pred_local}-{pred.pred_visitante}
                                  </p>
                                </div>

                                <div style={{ textAlign: 'center' }}>
                                  <p style={{ fontSize: '.7rem', color: '#6e6f73', margin: '0 0 .2rem' }}>Real</p>
                                  <p style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: '1.2rem', fontWeight: 700, color: pred.goles_local == null ? '#a9a9ae' : '#1f1f23', margin: 0 }}>
                            ...
                            <p style={{ fontSize: '.75rem', color: '#a9a9ae', textAlign: 'center', margin: 0 }}>Sin predicciones</p>
                          )}
                        </div>
                      </div>
                    )}
