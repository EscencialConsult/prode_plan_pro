      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 16, position: 'relative', marginTop: 6 }}>
        ...
        {!loading && (
          <div style={{ display: 'flex', gap: 20, flexShrink: 0 }}>
            ...
            {meta.mi_posicion && <BannerStat n={`#${meta.mi_posicion.posicion}`} label="Tu pos." gold={isGlobal || isExcl} />}
          </div>
        )}
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, paddingTop: 12 }}>
          {otros.map((u, idx) => (
            <div key={u.user_id} style={{
              ...
              border: u.user_id === user?.id ? '1.5px solid #86C873' : '1px solid #e2eede',
              boxShadow: u.user_id === user?.id ? '0 0 0 1px rgba(134,200,115,.3), 0 2px 8px rgba(134,200,115,.12)' : 'none',
              borderRadius: 10,
              alignItems: 'center',
              transition: 'all .15s',
            }}
              ...
              <div style={{ minWidth: 0 }}>
                <p style={{ fontWeight: u.user_id === user?.id ? 700 : 500, fontSize: 11, color: u.user_id === user?.id ? '#86C873' : '#111811', margin: '0 0 1px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {u.nombre}
                  {u.user_id === user?.id && (
                    <span style={{
                      fontSize: 8,
