  ...
  return (
    <div className="rk-in">
      ...
      <div style={{ borderRadius: 14, marginBottom: 24, background: 'linear-gradient(125deg,#0c182b 0%,#1a3060 100%)', padding: '18px 22px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, position: 'relative' }}>
          <div>
            <span style={{ fontSize: 9, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.22em', color: 'rgba(227,6,19,.55)', display: 'block', marginBottom: 4 }}>RANKING GLOBAL ACUMULADO</span>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, paddingTop: 12 }}>
          {otros.map((u, idx) => (
            <div key={u.user_id} style={{
              ...
              gap: 10,
              padding: '9px 12px',
              background: u.user_id === user?.id ? 'rgba(227,6,19,.1)' : '#fff',
              border: u.user_id === user?.id ? '1.5px solid #E30613' : '1px solid #f5f3ee',
              boxShadow: u.user_id === user?.id ? '0 0 0 1px rgba(227,6,19,.3), 0 2px 8px rgba(227,6,19,.12)' : 'none',
              borderRadius: 10,
              alignItems: 'center',
              transition: 'all .15s',
            }}
              ...
              <div style={{ minWidth: 0 }}>
                <p style={{ fontWeight: u.user_id === user?.id ? 700 : 500, fontSize: 11, color: u.user_id === user?.id ? '#E30613' : '#0c182b', margin: '0 0 1px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {u.nombre}
                  {u.user_id === user?.id && (
                    <span style={{
                      fontSize: 8,
              ...
              <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-start' }}>
                {[{ v: u.aciertos_exactos, c: '#22c55e' }, { v: u.aciertos_diferencia || 0, c: '#E30613' }].map((x, i) => (
                  x.v > 0 && (
