    ...
    <div style={{ position: 'sticky', bottom: 12, marginTop: 12, zIndex: 10 }}>
      <div style={{ borderRadius: 13, overflow: 'hidden', boxShadow: '0 8px 32px rgba(31,31,35,.28)', border: '2px solid rgba(192,39,39,.45)' }}>
        <div style={{
          ...
          background: 'linear-gradient(90deg,#1f1f23,#2e2e33)',
        }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, paddingTop: 12 }}>
          {otros.map((u, idx) => (
            <div key={u.user_id} style={{
              ...
              gap: 10,
              padding: '9px 12px',
              background: u.user_id === user?.id ? 'rgba(192,39,39,.1)' : '#fff',
              border: u.user_id === user?.id ? '1.5px solid #c02727' : '1px solid #f0f0f2',
              boxShadow: u.user_id === user?.id ? '0 0 0 1px rgba(192,39,39,.3), 0 2px 8px rgba(192,39,39,.12)' : 'none',
              borderRadius: 10,
              alignItems: 'center',
              transition: 'all .15s',
            }}
              ...
              <div style={{ minWidth: 0 }}>
                <p style={{ fontWeight: u.user_id === user?.id ? 700 : 500, fontSize: 11, color: u.user_id === user?.id ? '#c02727' : '#1f1f23', margin: '0 0 1px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {u.nombre}
                  {u.user_id === user?.id && (
                    <span style={{
                      fontSize: 8,
