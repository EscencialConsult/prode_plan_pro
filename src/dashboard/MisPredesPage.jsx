...
const CARD={background:'#fff',border:'1px solid #e4e4e7',borderRadius:14,boxShadow:'0 1px 0 rgba(31,31,35,.04)'}
const MUTED={fontSize:'.76rem',color:'#6e6f73'}
...
function StatCard({label,value,color='#1f1f23'}){
  return(
    <div style={{...CARD,padding:'1.1rem',textAlign:'center'}}>
      ...
      <p style={{fontSize:'.65rem',fontWeight:700,textTransform:'uppercase',letterSpacing:'.12em',color:'#a9a9ae',margin:'.25rem 0 0'}}>{label}</p>
    </div>
  )
}
...
function PartidoRow({match,pred}){
  ...
  return(
    <div style={{padding:'.7rem .85rem',borderRadius:10,background:pts>0?'rgba(27,138,90,.04)':'rgba(31,31,35,.02)',border:`1px solid ${pts>0?'rgba(27,138,90,.2)':'#e4e4e7'}`}}>
      {/* Equipos */}
      <div style={{display:'grid',gridTemplateColumns:'1fr auto 1fr',alignItems:'center',gap:'.5rem',marginBottom:'.45rem'}}>
        <div style={{display:'flex',alignItems:'center',gap:'.4rem',minWidth:0}}>
          {match.bandera_local&&<img src={match.bandera_local} alt="" style={{width:22,height:16,objectFit:'cover',borderRadius:2,border:'1px solid #e4e4e7',flexShrink:0}}/>}
          <span style={{fontWeight:600,fontSize:'.82rem',color:'#1f1f23',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{match.equipo_local}</span>
        </div>
        <span style={{fontSize:'.72rem',color:'#a9a9ae'}}>vs</span>
        <div style={{display:'flex',alignItems:'center',gap:'.4rem',minWidth:0,justifyContent:'flex-end'}}>
          <span style={{fontWeight:600,fontSize:'.82rem',color:'#1f1f23',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',textAlign:'right'}}>{match.equipo_visitante}</span>
          {match.bandera_visitante&&<img src={match.bandera_visitante} alt="" style={{width:22,height:16,objectFit:'cover',borderRadius:2,border:'1px solid #e4e4e7',flexShrink:0}}/>}
        </div>
      </div>

      {/* Scores */}
      <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:'1.2rem',paddingTop:'.45rem',borderTop:'1px solid #f0f0f2',flexWrap:'wrap'}}>
        {pred?(
          <div style={{textAlign:'center'}}>
            <p style={{fontSize:'.58rem',fontWeight:700,textTransform:'uppercase',letterSpacing:'.1em',color:'#a9a9ae',margin:'0 0 2px'}}>{pred.es_grupal?'Área':'Tu predicción'}</p>
            <p style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:'1.2rem',color:'#9e1f1f',margin:0,lineHeight:1}}>{pred.pred_local} - {pred.pred_visitante}</p>
          </div>
        ):(
        ...
        {(fin||live)&&match.goles_local!=null&&(
          <>
            ...
            <div style={{textAlign:'center'}}>
              <p style={{fontSize:'.58rem',fontWeight:700,textTransform:'uppercase',letterSpacing:'.1em',color:'#a9a9ae',margin:'0 0 2px'}}>{live?'En vivo':'Resultado'}</p>
              <p style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:'1.2rem',color:live?'#e03252':'#1f1f23',margin:0,lineHeight:1}}>{match.goles_local} - {match.goles_visitante}</p>
              {tienePenales && (
                <p style={{fontSize:'.6rem',fontWeight:700,color:'#9e1f1f',margin:'2px 0 0',textTransform:'uppercase',letterSpacing:'.06em'}}>
        ...
        {pts!=null&&pts!==''&&(
          <>
            <div style={{width:1,height:28,background:'#e4e4e7'}}/>
            <div style={{textAlign:'center'}}>
              <p style={{fontSize:'.58rem',fontWeight:700,textTransform:'uppercase',letterSpacing:'.1em',color:'#a9a9ae',margin:'0 0 2px'}}>Puntos</p>
              <p style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:'1.2rem',color:Number(pts)>0?'#1b8a5a':'#a9a9ae',margin:0,lineHeight:1}}>+{pts}</p>
            </div>
          </>
        )}
      </div>
      ...
      {esElim && (clasifPred || clasifRealCode) && (
        <div style={{marginTop:'.45rem',paddingTop:'.45rem',borderTop:'1px dashed #e4e4e7',display:'flex',flexWrap:'wrap',gap:'.4rem .9rem',alignItems:'center',justifyContent:'center'}}>
          {clasifPred && (
            <span style={{display:'inline-flex',alignItems:'center',gap:'.3rem',fontSize:'.7rem',fontFamily:"'DM Sans',sans-serif"}}>
              <span style={{fontSize:'.58rem',fontWeight:700,textTransform:'uppercase',letterSpacing:'.08em',color:'#a9a9ae'}}>
                Tu clasif.:
              </span>
              <span style={{fontWeight:700, color: clasifRealCode ? (aciertoClasif ? '#1b8a5a' : '#e03252') : '#9e1f1f'}}>
          ...
          {clasifRealCode && (
            <span style={{display:'inline-flex',alignItems:'center',gap:'.3rem',fontSize:'.7rem',fontFamily:"'DM Sans',sans-serif"}}>
              ...
              <span style={{fontWeight:700, color:'#1f1f23'}}>{nombreDesdeCodigo(match, clasifRealCode)}</span>
            </span>
          )}
        </div>
      )}
    ...
    <AppShell>
      ...
      <div style={{maxWidth:1400,margin:'0 auto',padding:'2rem 1.5rem 3rem'}}>
        ...
        <div className="din" style={{marginBottom:'1.6rem'}}>
          <h1 style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:'clamp(2.4rem,6vw,3.5rem)',color:'#1f1f23',margin:'0 0 .3rem',lineHeight:1,letterSpacing:'.02em'}}>MIS PREDICCIONES</h1>
        ...
        <div className="din" style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))',gap:'.85rem',marginBottom:'1.75rem',animationDelay:'60ms'}}>
          <StatCard label="Puntos totales" value={loading?'—':totalPts} color="#9e1f1f"/>
          <StatCard label="Predicciones" value={loading?'—':totalPred} color="#6e6f73"/>
          <StatCard label="Exactos" value={loading?'—':totalExactos} color="#1b8a5a"/>
          <StatCard label="Apuestas" value={loading?'—':betsConPred.length} color="#1f1f23"/>
        </div>
        ...
        <div className="din" style={{display:'flex',gap:'.3rem',marginBottom:'1.4rem',animationDelay:'80ms'}}>
          {[{k:'todas',l:'Todas'},{k:'con',l:`Con predicción (${betsConPred.length})`},{k:'sin',l:`Sin predicción (${betsSinPred.length})`}].map(({k,l})=>(
            <button key={k} onClick={()=>setFiltro(k)} style={{padding:'.34rem .82rem',borderRadius:99,fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:'.72rem',textTransform:'uppercase',letterSpacing:'.05em',cursor:'pointer',transition:'all .15s',background:filtro===k?'#1f1f23':'#fff',color:filtro===k?'#c02727':'#6e6f73',border:filtro===k?'1px solid transparent':'1px solid #e4e4e7',boxShadow:filtro===k?'0 2px 8px rgba(31,31,35,.18)':'none'}}>
              {l}
            </button>
          ))}
        </div>
        ...
        {(loading||lb)?(
          ...
          <div style={{...CARD,borderRadius:16,padding:'3rem',textAlign:'center'}}>
            <p style={{fontWeight:600,color:'#6e6f73',margin:0}}>No hay apuestas en esta categoría</p>
          </div>
        ):(
              ...
              return(
                <div key={bet.id} style={{...CARD,borderRadius:16,overflow:'hidden'}}>
                  ...
                  <div style={{padding:'1rem 1.2rem',display:'flex',alignItems:'center',gap:'1rem',cursor:'pointer'}}
                    ...
                    onMouseEnter={e=>{e.currentTarget.style.background='rgba(31,31,35,.02)'}}
                    onMouseLeave={e=>{e.currentTarget.style.background='transparent'}}>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:'flex',alignItems:'center',gap:'.55rem',marginBottom:'.22rem'}}>
                        <h3 style={{fontWeight:700,fontSize:'.92rem',color:'#1f1f23',margin:0,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{bet.titulo}</h3>
                    ...
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#a9a9ae" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{flexShrink:0,transition:'transform .2s',transform:isExpanded?'rotate(180deg)':'rotate(0)'}}>
                  ...
                  {/* Expandido */}
                  {isExpanded&&(
                    <div style={{padding:'0 1.2rem 1.2rem',borderTop:'1px solid #f0f0f2'}}>
                      ...
                      {isOpenBet&&(
                        <Link to="/apuestas" style={{display:'block',textAlign:'center',marginTop:'1rem',padding:'.55rem',fontFamily:"'DM Sans',sans-serif",fontSize:'.8rem',fontWeight:700,color:'#9e1f1f',textDecoration:'none',borderRadius:99,border:'1px solid rgba(192,39,39,.3)',background:'rgba(192,39,39,.06)',transition:'all .17s'}}
                          onMouseEnter={e=>{e.currentTarget.style.background='rgba(192,39,39,.12)'}}
                          onMouseLeave={e=>{e.currentTarget.style.background='rgba(192,39,39,.06)'}}>
                          Editar predicciones →
                        </Link>
                      )}
                    </div>
