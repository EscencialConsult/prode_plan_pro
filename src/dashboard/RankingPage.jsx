...
/* Panel izquierdo */
.rk-sidebar { width:380px;flex-shrink:0;display:flex;flex-direction:column;overflow:hidden;background:#f0f5ee;border-right:1px solid #c8dbcc }
...
.rk-trow:hover { background:rgba(17,24,17,.03) !important }
...
@media(max-width:720px) {
  .rk-shell { flex-direction:column!important;height:auto!important;min-height:65vh!important }
  .rk-sidebar { width:100%;max-height:none!important;border-right:none!important;border-bottom:none!important }
  ...
  .rk-shell.has-sel .rk-sidebar { display: none !important }
  .rk-shell.no-sel .rk-content { display: none !important }
}
`
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '2rem 1.5rem 3rem' }}>
        ...
        <div className="rk-in" style={{ marginBottom: '1.25rem' }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 12 }}>
            <h1 style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 'clamp(2.4rem,6vw,3.5rem)', color: '#111811', margin: '0 0 .3rem', lineHeight: 1, letterSpacing: '.02em' }}>RANKING</h1>
            {sel && <span style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 13, color: '#94a3b8', fontWeight: 500 }}>— {sel.titulo}</span>}
          </div>
        </div>

        {/* Shell principal */}
        <div className={`rk-shell ${sel ? 'has-sel' : 'no-sel'}`} style={{ display: 'flex', height: 'calc(100vh - 162px)', minHeight: 520, borderRadius: 20, overflow: 'hidden', boxShadow: '0 8px 48px rgba(17,24,17,.14)' }}>
          {/* ══ SIDEBAR OSCURO ══ */}
          ...
          <div className="rk-content" style={{ padding: '24px 32px 32px' }}>
            ...
            {!sel ? (
              ...
              <div className="rk-in flex flex-col">
                
                {/* Mobile Back Button */}
                <button 
                  onClick={() => setSel(null)}
                  className="md:hidden flex items-center gap-2 mb-4 font-body font-bold text-xs uppercase tracking-wider"
                  style={{ color: '#4a6b50', width: 'fit-content' }}>
  ...
  return (
    <div style={{ position: 'relative', maxWidth: 240, marginBottom: 16 }}>
      ...
      <input value={q} onChange={e => setQ(e.target.value)} placeholder="Buscar participante..."
        style={{ width: '100%', paddingLeft: 28, paddingRight: q ? 28 : 10, paddingTop: 7, paddingBottom: 7, borderRadius: 99, border: '1.5px solid #c8dbcc', background: '#fff', fontFamily: "'DM Sans',sans-serif", fontSize: 12, color: '#111811', outline: 'none', boxSizing: 'border-box', transition: 'border-color .15s' }}
        onFocus={e => e.target.style.borderColor = 'rgba(134,200,115,.6)'}
        onBlur={e => e.target.style.borderColor = '#c8dbcc'}
      />
      {q && (
        ...
        {orden.map(u => {
          ...
          return (
            <div key={u.user_id} className="rk-pcard"
              ...
              style={{
                background: '#fff',
                border: `${isTop || isExp ? 2 : 1.5}px solid ${isExp ? '#86C873' : isTop ? cfg.border : '#e8e3db'}`,
                padding: isTop ? '20px 14px 14px' : '16px 12px 12px',
                boxShadow: isTop
                  ? `0 0 0 4px ${cfg.ring}, 0 12px 40px ${cfg.shadow}`
                  : isExp ? `0 0 0 3px rgba(134,200,115,.2)` : '0 2px 12px rgba(17,24,17,.06)',
              ...
              <div style={{
                ...
                border: isTop ? '1px solid rgba(134,200,115,.25)' : '1px solid #c8dbcc',
              ...
              {/* CTA */}
              {puede && (
                <button style={{ marginTop: 10, background: 'transparent', border: 'none', cursor: 'pointer', fontSize: 10, fontWeight: 700, color: isExp ? '#5A9E4A' : '#94a3b8', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 3, width: '100%', padding: '4px 0', transition: 'color .14s' }}
                  onMouseEnter={e => e.currentTarget.style.color = '#5A9E4A'}
                  onMouseLeave={e => e.currentTarget.style.color = isExp ? '#5A9E4A' : '#94a3b8'}>
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      ...
      <div style={{ borderRadius: 14, overflow: 'hidden', border: '1.5px solid #e8e3db', background: '#fff' }}>
        {/* Header */}
        <div style={{ display: 'grid', gridTemplateColumns: '44px 1fr 100px 68px 18px', padding: '8px 16px', background: 'rgba(17,24,17,.02)', borderBottom: '1px solid #c8dbcc', gap: 8, alignItems: 'center' }}>
        ...
        {filas.map((u, i) => {
          ...
          return (
            <div key={u.user_id} style={{ borderBottom: i < filas.length - 1 ? '1px solid #e2eede' : 'none' }}>
              <div className="rk-trow"
                onClick={puede ? () => onToggle(u.user_id) : undefined}
                style={{
                  display: 'grid', gridTemplateColumns: '44px 1fr 100px 68px 18px',
                  padding: '11px 16px', gap: 8, alignItems: 'center',
                  background: me ? 'rgba(134,200,115,.04)' : isExp ? 'rgba(17,24,17,.015)' : '#fff',
                  cursor: puede ? 'pointer' : 'default',
                }}>

                {/* Posición */}
                <span style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 18, color: pos <= 3 ? '#86C873' : '#c8d0dc', lineHeight: 1 }}>{pos}</span>

                {/* Participante */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
                  <div style={{
                    width: 34, height: 34, borderRadius: '50%', flexShrink: 0,
                    background: me ? 'linear-gradient(135deg,#86C873,#5A9E4A)' : 'linear-gradient(135deg,#e2e8f0,#cbd5e1)',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontFamily: "'Bebas Neue',sans-serif", fontSize: 13,
                    color: me ? '#111811' : '#64748b',
                    boxShadow: me ? '0 0 0 2px rgba(134,200,115,.3)' : 'none',
                  }}>{initials(u.nombre)}</div>
                  <div style={{ minWidth: 0 }}>
                    <p style={{ fontWeight: me ? 700 : 600, fontSize: 13, color: me ? '#5A9E4A' : '#111811', margin: '0 0 1px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {u.nombre}
                      {me && <span style={{ fontSize: 10, color: '#94a3b8', fontWeight: 400, marginLeft: 4 }}>(vos)</span>}
                    </p>
                ...
                <div className="rk-hide-mob" style={{ display: 'flex', justifyContent: 'center', gap: 8 }}>
                  {[['#22c55e', u.aciertos_exactos, 'Exactos'], ['#86C873', u.aciertos_diferencia || 0, 'Dif.'], ['#94a3b8', u.aciertos_resultado, 'Res.']].map(([c, n, t]) => (
                ...
                {/* Puntos */}
                <div style={{ textAlign: 'center' }}>
                  <span style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 20, color: '#111811', lineHeight: 1 }}>{u.puntos_totales}</span>
                  <span style={{ fontSize: 8, color: '#94a3b8', marginLeft: 2, letterSpacing: '.1em', fontWeight: 700, display: 'block' }}>PTS</span>
                </div>

                {/* Chevron */}
                {puede
                  ? <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke={isExp ? '#86C873' : '#cbd5e1'} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ transform: isExp ? 'rotate(180deg)' : 'none', transition: 'transform .2s' }}>
                    <polyline points="6 9 12 15 18 9" />
                  </svg>
                  : <span />
                }
...
function DetallePanel({ nombre, det, apuesta, onClose, inline, style: extraStyle }) {
  const base = {
    ...
    overflow: 'hidden',
    border: inline ? 'none' : '2px solid rgba(134,200,115,.4)',
    borderTop: inline ? '2px solid rgba(134,200,115,.25)' : undefined,
    boxShadow: inline ? 'none' : '0 4px 24px rgba(134,200,115,.1)',
  ...
  const loadingUI = (
    <div style={base}>
      <div style={{ background: '#111811', padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.14em', color: 'rgba(134,200,115,.7)' }}>Cargando…</span>
      </div>
      <div style={{ background: '#f0f5ee', padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 4 }}>
  ...
  if (det.error) return (
    <div style={base}>
      <div style={{ background: '#111811', padding: '10px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.14em', color: '#f43f5e' }}>Error al cargar</span>
        <CloseBtn onClick={onClose} />
      </div>
      <div style={{ background: '#f0f5ee', padding: '12px 14px', fontSize: 12, color: '#f43f5e' }}>{det.error}</div>
    </div>
  )
  ...
  return (
    <div style={base}>
      {/* Header */}
      <div style={{ background: 'linear-gradient(90deg,#111811,#263328)', padding: '10px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ width: 24, height: 24, borderRadius: 6, background: 'rgba(134,200,115,.15)', border: '1px solid rgba(134,200,115,.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Bebas Neue',sans-serif", fontSize: 10, color: '#86C873' }}>
            {initials(nombre)}
          </div>
          <span style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.12em', color: 'rgba(134,200,115,.85)' }}>{nombre}</span>
        </div>
        <CloseBtn onClick={onClose} />
      </div>

      {/* Body */}
      <div style={{ background: '#f0f5ee', padding: '12px 14px 14px' }}>
        {/* Resumen chips */}
        {det.preds?.length > 0 && (
          <div style={{ display: 'flex', gap: 6, marginBottom: 12, flexWrap: 'wrap', alignItems: 'center' }}>
            {[['exacto', '#22c55e', 'Exactos'], ['diferencia', '#86C873', 'Diferencia'], ['resultado', '#94a3b8', 'Resultado'], ['fallos', '#f43f5e', 'Fallos']].map(([k, c, l]) => resumen[k] > 0 && (
            ...
            {resumen.total > 0 && (
              <span style={{ marginLeft: 'auto', fontFamily: "'Bebas Neue',sans-serif", fontSize: 18, color: '#111811' }}>
    ...
    <div style={{
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, overflow: 'hidden', marginBottom: 2 }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 600, color: '#111811', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{partido.equipo_local}</span>
          <span style={{ fontSize: 9, color: '#b8c0cc', flexShrink: 0 }}>vs</span>
          <span style={{ fontSize: 11, fontWeight: 600, color: '#111811', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{partido.equipo_visitante}</span>
      ...
      <div style={{ textAlign: 'center' }}>
        ...
        {pred
          ? <p style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 14, color: '#5A9E4A', margin: 0, lineHeight: 1 }}>{pred.pred_local}-{pred.pred_visitante}</p>
      ...
      <div style={{ textAlign: 'center' }}>
        ...
        {tieneScore
          ? <p style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 14, color: '#111811', margin: 0, lineHeight: 1 }}>{partido.goles_local}-{partido.goles_visitante}</p>
    ...
    <div style={{ position: 'sticky', bottom: 12, marginTop: 12, zIndex: 10 }}>
      <div style={{ borderRadius: 13, overflow: 'hidden', boxShadow: '0 8px 32px rgba(17,24,17,.28)', border: '2px solid rgba(134,200,115,.45)' }}>
        <div onClick={onToggle} style={{
          ...
          background: 'linear-gradient(90deg,#111811,#263328)', cursor: 'pointer',
        }}>
          <span style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 18, color: '#86C873' }}>#{pos.posicion}</span>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
            <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'linear-gradient(135deg,#86C873,#5A9E4A)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Bebas Neue',sans-serif", fontSize: 12, color: '#111811', flexShrink: 0 }}>
              {initials(pos.nombre)}
            </div>
            <div style={{ minWidth: 0 }}>
              <p style={{ fontWeight: 700, fontSize: 13, color: '#fff', margin: '0 0 1px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {pos.nombre}<span style={{ fontSize: 10, color: '#86C873', fontWeight: 400, marginLeft: 4 }}>(vos)</span>
              </p>
              <p style={{ fontSize: 10, color: 'rgba(255,255,255,.4)', margin: 0 }}>{pos.predicciones} predicciones</p>
            </div>
          </div>
          <div className="rk-hide-mob" style={{ display: 'flex', justifyContent: 'center', gap: 8 }}>
            {[['#22c55e', pos.aciertos_exactos], ['#86C873', pos.aciertos_diferencia || 0], ['rgba(255,255,255,.45)', pos.aciertos_resultado]].map(([c, n], i) => (
          ...
          <div style={{ textAlign: 'center' }}>
            <span style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 20, color: '#86C873' }}>{pos.puntos_totales}</span>
            <span style={{ fontSize: 8, color: 'rgba(255,255,255,.3)', marginLeft: 2, letterSpacing: '.1em', fontWeight: 700, display: 'block' }}>PTS</span>
          </div>
          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#86C873" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"
            style={{ transform: expandida ? 'rotate(180deg)' : 'none', transition: 'transform .2s' }}>
            <polyline points="6 9 12 15 18 9" />
          </svg>
        </div>
        {expandida && (
          <div style={{ background: '#f0f5ee' }}>
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 40, textAlign: 'center', gap: 20 }}>
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end', opacity: .15, marginTop: 8, pointerEvents: 'none' }}>
        {[80, 110, 80].map((h, i) => (
          <div key={i} style={{ width: 56, height: h, borderRadius: 12, background: `linear-gradient(180deg,${i === 1 ? '#86C873' : '#cbd5e1'},transparent)` }} />
        ))}
      </div>
    </div>
  )
...
function SinResultados({ q, clear }) {
  return (
    <div style={{ textAlign: 'center', padding: 32, background: '#fff', borderRadius: 14, border: '1.5px solid #e8e3db', marginBottom: 12 }}>
      ...
      <button onClick={clear} style={{ background: 'transparent', border: '1.5px solid #c8dbcc', borderRadius: 99, padding: '5px 16px', fontSize: 12, fontWeight: 600, color: '#64748b', cursor: 'pointer', transition: 'all .14s' }}
        onMouseEnter={e => { e.currentTarget.style.borderColor = '#111811'; e.currentTarget.style.color = '#111811' }}
        onMouseLeave={e => { e.currentTarget.style.borderColor = '#c8dbcc'; e.currentTarget.style.color = '#64748b' }}>
        Limpiar búsqueda
      </button>
    </div>
  )
