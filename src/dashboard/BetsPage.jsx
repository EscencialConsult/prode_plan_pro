...
const CARD_BASE={background:'#fff',border:'1px solid #e4e4e7',borderRadius:16,boxShadow:'0 1px 0 rgba(31,31,35,.04)'}
const MUTED={fontSize:'.78rem',color:'#6e6f73'}
...
const STATE={
  ...
  cerrada:   {label:'CERRADA',  color:'#6e6f73', bg:'rgba(95,110,138,.07)',border:'rgba(95,110,138,.18)'},
  finalizada:{label:'FINALIZADA',color:'#9e1f1f',bg:'rgba(192,39,39,.08)',border:'rgba(192,39,39,.22)'},
}
...
function Toast({msg,ok}){
  if(!msg)return null
  return(
    <div style={{position:'fixed',top:'1.2rem',right:'1.2rem',zIndex:999,padding:'.75rem 1.1rem',borderRadius:12,background:ok?'#1f1f23':'#fff',border:`1px solid ${ok?'rgba(192,39,39,.4)':'rgba(224,50,82,.3)'}`,boxShadow:'0 8px 28px rgba(31,31,35,.15)',display:'flex',alignItems:'center',gap:'.55rem',fontFamily:"'DM Sans',sans-serif",fontSize:'.84rem',fontWeight:600,color:ok?'#c02727':'#e03252',animation:'tin .3s ease both'}}>
      {ok
...
function BetCard({bet,predsMap,onPredict}){
  ...
  return(
    <div style={{...CARD_BASE,borderRadius:18,padding:'1.4rem 1.5rem',border:`1px solid ${live?'rgba(224,50,82,.25)':open?'rgba(27,138,90,.18)':'#e4e4e7'}`,transition:'transform .2s,box-shadow .2s'}}
      onMouseEnter={e=>{e.currentTarget.style.transform='translateY(-2px)';e.currentTarget.style.boxShadow='0 10px 28px rgba(31,31,35,.1)'}}
      ...
      <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:'1rem',marginBottom:'1rem'}}>
        <div style={{flex:1,minWidth:0}}>
          <h3 style={{fontWeight:700,fontSize:'1rem',color:'#1f1f23',margin:'0 0 .3rem',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{bet.titulo}</h3>
      ...
      {bet.partidos?.length>0&&(
        <div style={{display:'flex',flexDirection:'column',gap:'.4rem',marginBottom:'1.1rem'}}>
          {bet.partidos.slice(0,3).map(m=>{
            ...
            return(
              <div key={m.id} style={{display:'grid',gridTemplateColumns:'1fr auto 1fr',alignItems:'center',gap:'.6rem',padding:'.55rem .75rem',borderRadius:9,background:pred?'rgba(192,39,39,.05)':'rgba(31,31,35,.02)',border:pred?'1px solid rgba(192,39,39,.2)':'1px solid #e4e4e7'}}>
                <span style={{fontWeight:500,fontSize:'.8rem',color:'#1f1f23',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{m.equipo_local}</span>
                <div style={{textAlign:'center',minWidth:60}}>
                  {pred?(
                    <span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:'1.05rem',color:'#9e1f1f',letterSpacing:'.04em'}}>{pred.pred_local} - {pred.pred_visitante}</span>
                  ):(
                    <span style={{fontSize:'.75rem',color:'#a9a9ae'}}>— vs —</span>
                  )}
                  {fin&&m.goles_local!=null&&(
                    <div style={{fontSize:'.6rem',color:'#6e6f73',marginTop:1}}>Real: {m.goles_local}-{m.goles_visitante}</div>
                  )}
                </div>
                <span style={{fontWeight:500,fontSize:'.8rem',color:'#1f1f23',textAlign:'right',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{m.equipo_visitante}</span>
              </div>
            )
          })}
      ...
      {/* Footer */}
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:'1rem',paddingTop:'.85rem',borderTop:'1px solid #e4e4e7'}}>
        ...
        {canPredict&&(
          <button onClick={()=>onPredict(bet)} style={{fontWeight:700,fontSize:'.8rem',padding:'.55rem 1.1rem',borderRadius:99,border:'none',background:'#c02727',color:'#ffffff',cursor:'pointer',transition:'all .17s',boxShadow:'0 4px 14px rgba(192,39,39,.25)',whiteSpace:'nowrap'}}
...
<p className="font-body text-sm" style={{ color: '#6e6f73' }}>
  {filtered.length} {filtered.length === 1 ? 'apuesta disponible' : 'apuestas disponibles'}
        ...
        <div className="mb-6 animate-fade-in bg-white rounded-2xl border border-[#e4e4e7] overflow-hidden">

          {/* Cabecera */}
          <button
            onClick={() => setShowPuntos(v => !v)}
            className="w-full flex items-center justify-between px-5 py-4 bg-transparent border-none cursor-pointer transition-all hover:bg-[rgba(31,31,35,.02)]"
            style={{ borderBottom: showPuntos ? '1px solid #e4e4e7' : 'none' }}
          >
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0"
                style={{ background: 'linear-gradient(135deg,#1f1f23,#6e6f73)' }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#c02727" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/>
                </svg>
              </div>
              <div className="text-left">
                <p className="font-body font-bold text-sm text-[#1f1f23] m-0">Sistema de puntos</p>
                <p className="font-body text-xs text-[#6e6f73] m-0">Así se calcula tu puntaje en cada partido</p>
              </div>
            </div>
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#a9a9ae" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"
          ...
          {showPuntos && (
            <div className="px-5 pb-5 pt-4">
              {/* Bloque grupos */}
              <p className="font-body font-bold uppercase mb-2" style={{ fontSize: '.6rem', letterSpacing: '.12em', color: '#6e6f73' }}>
                Fase de grupos
              </p>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {[
                  { pts: 1, titulo: 'Resultado',       desc: 'Acertás quién gana, pierde o empata',           accent: '#6e6f73', bg: 'rgba(110,111,115,.06)',  border: 'rgba(110,111,115,.2)'   },
                  { pts: 3, titulo: 'Res. + diferencia', desc: 'Acertás resultado y diferencia de goles',      accent: '#1f1f23', bg: 'rgba(31,31,35,.04)',    border: 'rgba(31,31,35,.12)'   },
                  { pts: 5, titulo: 'Marcador exacto',  desc: 'Acertás el resultado final exacto del partido', accent: '#9e1f1f', bg: 'rgba(192,39,39,.07)',  border: 'rgba(192,39,39,.28)' },
                  ...
                  <div key={pts} className="flex items-center gap-3 rounded-xl p-4"
                    ...
                    <div>
                      <p className="font-body font-bold text-sm m-0 mb-1" style={{ color: '#1f1f23' }}>{titulo}</p>
                      <p className="font-body text-xs leading-snug m-0" style={{ color: '#6e6f73' }}>{desc}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <p className="font-body font-bold uppercase mt-5 mb-2" style={{ fontSize: '.6rem', letterSpacing: '.12em', color: '#6e6f73' }}>
              ...
              <div className="rounded-xl p-4" style={{ background: 'rgba(244,180,42,.06)', border: '1px solid rgba(244,180,42,.25)' }}>
                <p className="font-body text-xs mb-3" style={{ color: '#1f1f23' }}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  {[
                    ...
                    <div key={titulo} className="flex items-center gap-3 rounded-lg p-2.5" style={{ background: '#fff', border: '1px solid #e4e4e7' }}>
                      <div className="flex flex-col items-center justify-center flex-shrink-0"
                        style={{ width: 38, height: 38, borderRadius: 8, background: 'rgba(244,180,42,.1)', border: '1px solid rgba(244,180,42,.3)' }}>
                        <span className="font-display leading-none" style={{ fontSize: '1.25rem', color: '#9e1f1f' }}>{pts}</span>
                      </div>
                      <div className="min-w-0">
                        <p className="font-body font-bold text-xs m-0 mb-0.5" style={{ color: '#1f1f23' }}>{titulo}</p>
                        <p className="font-body m-0" style={{ fontSize: '.7rem', color: '#6e6f73', lineHeight: 1.3 }}>{desc}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <p className="font-body text-center mt-3" style={{ fontSize: '.68rem', color: '#a9a9ae' }}>
        ...
        {loading?(
          <div style={{display:'grid',gap:'1rem'}}>
            {[1,2,3].map(i=><div key={i} style={{height:200,borderRadius:18,background:'#fff',border:'1px solid #e4e4e7',animation:'skp 1.4s ease-in-out infinite'}}/>)}
          </div>
        ):filtered.length===0?(
          <div style={{borderRadius:18,padding:'3.5rem 1.5rem',textAlign:'center',background:'#fff',border:'1.5px dashed #e4e4e7'}}>
            <svg width="44" height="44" viewBox="0 0 24 24" fill="none" stroke="#a9a9ae" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{margin:'0 auto 1rem'}}><path d="M6 9H4.5a2.5 2.5 0 0 1 0-5H6"/><path d="M18 9h1.5a2.5 2.5 0 0 0 0-5H18"/><path d="M4 22h16"/><path d="M18 2H6v7a6 6 0 0 0 12 0V2Z"/></svg>
            <p style={{fontWeight:600,fontSize:'.92rem',color:'#6e6f73',margin:0}}>{filter==='activas'?'No hay apuestas activas':filter==='cerradas'?'No hay apuestas cerradas':'No hay apuestas publicadas'}</p>
            <p style={{fontSize:'.78rem',color:'#a9a9ae',margin:'.4rem 0 0'}}>Volvé a revisar más tarde</p>
          </div>
        ):(
