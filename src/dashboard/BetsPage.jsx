      ...
      {bet.partidos?.length>0&&(
        <div style={{display:'flex',flexDirection:'column',gap:'.4rem',marginBottom:'1.1rem'}}>
          {bet.partidos.slice(0,3).map(m=>{
            ...
            const esTBD=nombreLocal==='Por confirmar'||nombreVisitante==='Por confirmar'
            return(
              <div key={m.id} style={{display:'grid',gridTemplateColumns:'1fr auto 1fr',alignItems:'center',gap:'.6rem',padding:'.55rem .75rem',borderRadius:9,background:esTBD?'rgba(251,191,36,.06)':pred?'rgba(125,211,252,.05)':'rgba(12,24,43,.02)',border:esTBD?'1px solid rgba(251,191,36,.3)':pred?'1px solid rgba(125,211,252,.2)':'1px solid #f0eadb'}}>
                <span style={{fontWeight:500,fontSize:'.8rem',color:esTBD?'#92400e':'#0c182b',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{nombreLocal}</span>
                <div style={{textAlign:'center',minWidth:60}}>
                  {esTBD?(
                    <span style={{fontSize:'.65rem',color:'#b45309',fontWeight:600}}>⏳ TBD</span>
                  ):pred?(
                ...
                <span style={{fontWeight:500,fontSize:'.8rem',color:esTBD?'#92400e':'#0c182b',textAlign:'right',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{nombreVisitante}</span>
              </div>
            )
          })}
