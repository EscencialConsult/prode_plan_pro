...
const STATE={
  ...
  finalizada:{label:'FINALIZADA',color:'#0057B8',bg:'rgba(235,195,43,.08)',border:'rgba(235,195,43,.22)'},
}
...
function Toast({msg,ok}){
  if(!msg)return null
  return(
    <div style={{position:'fixed',top:'1.2rem',right:'1.2rem',zIndex:999,padding:'.75rem 1.1rem',borderRadius:12,background:ok?'#0c182b':'#fff',border:`1px solid ${ok?'rgba(0,87,184,.4)':'rgba(224,50,82,.3)'}`,boxShadow:'0 8px 28px rgba(12,24,43,.15)',display:'flex',alignItems:'center',gap:'.55rem',fontFamily:"'DM Sans',sans-serif",fontSize:'.84rem',fontWeight:600,color:ok?'#0057B8':'#e03252',animation:'tin .3s ease both'}}>
      {ok
  ...
  return(
    <div style={{...CARD_BASE,borderRadius:18,padding:'1.4rem 1.5rem',border:`1px solid ${live?'rgba(224,50,82,.25)':open?'rgba(27,138,90,.18)':'#f0eadb'}`,transition:'transform .2s,box-shadow .2s'}}
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:'1rem',paddingTop:'.85rem',borderTop:'1px solid #f0eadb'}}>
        ...
        {canPredict&&(
          <button onClick={()=>onPredict(bet)} style={{fontWeight:700,fontSize:'.8rem',padding:'.55rem 1.1rem',borderRadius:99,border:'none',background:'#0057B8',color:'#05090f',cursor:'pointer',transition:'all .17s',boxShadow:'0 4px 14px rgba(255,255,255,.25)',whiteSpace:'nowrap'}}
            ...
            <div className="px-5 pb-5 pt-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {[
                  ...
                  { pts: 5, titulo: 'Marcador exacto',  desc: 'Acertás el resultado final exacto del partido', accent: '#0057B8', bg: 'rgba(235,195,43,.07)',  border: 'rgba(235,195,43,.28)' },
                  ...
                  {[
                    ...
                    <div key={titulo} className="flex items-center gap-3 rounded-lg p-2.5" style={{ background: '#fff', border: '1px solid #f0eadb' }}>
                      <div className="flex flex-col items-center justify-center flex-shrink-0"
                        ...
                        <span className="font-display leading-none" style={{ fontSize: '1.25rem', color: '#0057B8' }}>{pts}</span>
                      </div>
