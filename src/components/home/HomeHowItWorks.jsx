...
export default function HomeHowItWorks() {
  return (
    ...
    <section id="como-funciona" className="relative" style={{ background: '#F7F7F7', paddingTop: '5rem', paddingBottom: '6rem' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 1.5rem' }}>
        ...
        <div className="text-center mb-14">
          <span className="inline-flex items-center gap-2 font-body font-bold text-xs uppercase tracking-widest px-4 py-1.5 rounded-full mb-5"
            style={{ border: '1px solid #E30613', color: '#A8030C', background: 'rgba(227,6,19,.1)' }}>
            Cómo funciona
          </span>
          <h2 className="font-display" style={{ fontSize: 'clamp(2.4rem,6vw,4rem)', color: '#0A0A0A', lineHeight: 1, letterSpacing: 0 }}>
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-12">
          {STEPS.map(({ n, title, desc, icon }) => (
            <div key={n} className="relative rounded-2xl p-7 bg-white transition-all duration-300"
              style={{ border: '1px solid #E6E6E6', boxShadow: '0 1px 0 rgba(0,0,0,.03)' }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = '#E30613'; e.currentTarget.style.transform = 'translateY(-4px)'; e.currentTarget.style.boxShadow = '0 14px 36px rgba(0,0,0,.1)' }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = '#E6E6E6'; e.currentTarget.style.transform = ''; e.currentTarget.style.boxShadow = '0 1px 0 rgba(0,0,0,.03)' }}
            >
              <div className="font-display leading-none mb-4 select-none" style={{ fontSize: '5.5rem', color: '#E30613', opacity: .70 }}>
                {n}
              </div>
              <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-5"
                style={{ background: 'linear-gradient(135deg,#0A0A0A,#333333)', color: '#E30613' }}>
                {icon}
              </div>
              <h3 className="font-display text-2xl mb-3" style={{ color: '#0A0A0A', letterSpacing: 0 }}>{title}</h3>
              <p className="font-body text-sm leading-relaxed" style={{ color: '#555555' }}>{desc}</p>
            </div>
          ))}
        </div>
        ...
        <div className="text-center">
          <Link to="/register"
            ...
            style={{ background: '#E30613', color: '#fff', boxShadow: '0 8px 24px rgba(227,6,19,.28)', textDecoration: 'none' }}
      ...
      {/* ── WAVE: howItWorks (#F7F7F7) → features (#0A0A0A)
      ...
      <svg className="absolute bottom-0 left-0 w-full" style={{ display: 'block', height: 72, marginBottom: -2 }}
        ...
        <rect width="1440" height="72" fill="#F7F7F7" />
        <path d="M0,40 C360,72 720,0 1080,30 C1260,45 1380,60 1440,40 L1440,72 L0,72 Z" fill="#0A0A0A" />
      </svg>
    </section>
  )
}