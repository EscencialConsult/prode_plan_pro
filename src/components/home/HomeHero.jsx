    ...
    <>
      ...
      <section className="relative min-h-screen text-white overflow-hidden flex items-center"
        ...
        <div className="absolute inset-0 pointer-events-none" style={{
          background: 'radial-gradient(ellipse 60% 50% at 85% 20%, rgba(125,211,252,.15), transparent 55%), radial-gradient(ellipse 50% 60% at 15% 90%, rgba(66,91,139,.28), transparent 60%)'
        }} />
          ...
          <div className="grid lg:grid-cols-12 gap-8 lg:gap-14 items-center">
            ...
            <div className="lg:col-span-7 space-y-5 sm:space-y-6">
              ...
              <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                <span className="inline-flex items-center gap-2 font-body font-bold text-xs uppercase tracking-widest px-3 py-1.5 rounded-full"
                  style={{ border: '1px solid #7dd3fc', color: '#7dd3fc', background: 'rgba(125,211,252,.1)' }}>
                ...
                <span className="hidden sm:flex items-center gap-2 text-xs font-body font-semibold uppercase tracking-widest" style={{ color: 'rgba(255,255,255,.55)' }}>
                  <span className="h-px w-6" style={{ background: 'rgba(125,211,252,.45)' }} />
                  CODINI, La vida es Simple.
                </span>
              </div>
              ...
              <div>
                ...
                <h1 className="font-display leading-none block" style={{ fontSize: 'clamp(2.8rem,12vw,7.5rem)', letterSpacing: '.01em', color: '#7dd3fc', textShadow: '0 0 40px rgba(125,211,252,.4)' }}>
                  SE VIVE
                </h1>
                <h1 className="font-display leading-none block text-white" style={{ fontSize: 'clamp(1.8rem,8vw,5rem)', letterSpacing: '.01em' }}>
                  ACÁ ADENTRO
              ...
              <div className="flex flex-col sm:flex-row sm:flex-wrap gap-2 sm:gap-3">
                {[
                  { label: 'Competencia sana', sub: 'Entre equipos y áreas', bg: 'linear-gradient(135deg,#7dd3fc,#0ea5e9)', ic: '#05090f' },
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <Link to="/register" className="font-body font-bold text-sm sm:text-base px-6 sm:px-7 py-3.5 sm:py-4 rounded-full inline-flex items-center justify-center gap-2 transition-all w-full sm:w-auto"
                  style={{ background: '#7dd3fc', color: '#05090f', boxShadow: '0 8px 24px rgba(125,211,252,.3)', textDecoration: 'none' }}
                  onMouseEnter={e => { e.currentTarget.style.background = '#bae6fd'; e.currentTarget.style.transform = 'translateY(-2px)' }}
            ...
            <div className="lg:col-span-5">
              ...
              <div className="relative mb-5 sm:mb-6 flex justify-center">
                <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(circle, rgba(125,211,252,.28), transparent 60%)', filter: 'blur(28px)' }} />
              ...
              <div className="rounded-2xl p-4 sm:p-5"
                style={{ background: 'linear-gradient(160deg,rgba(12,24,43,.78),rgba(5,9,15,.88))', border: '1px solid rgba(125,211,252,.28)', backdropFilter: 'blur(16px)', boxShadow: '0 28px 70px rgba(5,9,15,.55)' }}>
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="flex items-center gap-1.5 text-xs font-body font-bold uppercase tracking-widest" style={{ color: '#7dd3fc' }}>
                    <span className="w-1.5 h-1.5 rounded-full bg-green-400 pulse-dot-el" />
                    En vivo
                  </span>
                </div>

                <div className="mb-4 h-px" style={{ background: 'linear-gradient(90deg,transparent,rgba(125,211,252,.3),transparent)' }} />

                {/* Countdown */}
                <p className="font-body font-bold text-xs uppercase tracking-widest mb-3 text-center" style={{ color: '#7dd3fc' }}>
                ...
                <div className="grid grid-cols-4 gap-1.5 mb-4">
                  {[{ v: cd.days, l: 'Días' }, { v: cd.hours, l: 'Horas' }, { v: cd.minutes, l: 'Min' }, { v: cd.seconds, l: 'Seg' }].map(({ v, l }) => (
                    <div key={l} className="py-2 sm:py-3 text-center rounded-lg"
                      style={{ background: 'rgba(125,211,252,.06)', border: '1px solid rgba(125,211,252,.18)' }}>
                      <div className="font-display leading-none" style={{ fontSize: 'clamp(1.2rem,5vw,2rem)', color: '#7dd3fc' }}>
                ...
                <p className="font-body font-bold text-xs uppercase tracking-widest mb-3" style={{ color: '#7dd3fc' }}>Predicciones recientes</p>
                <div className="space-y-2.5 mb-4">
                  {PREDS.map(({ user, pred, pts }) => (
                    <div key={user} className="flex items-center justify-between gap-2">
                      ...
                      <span className="font-body font-bold text-xs flex-shrink-0" style={{ color: '#7dd3fc' }}>{pts}</span>
                    </div>
                  ))}
                </div>
