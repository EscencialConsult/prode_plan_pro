...
const FEATURES = [
  { title: 'Predicciones por partido', desc: 'Ingresa tu resultado antes del cierre. El sistema acepta pronósticos hasta el inicio del partido.', icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg> },
  { title: 'Ranking en tiempo real', desc: 'Posiciones actualizadas automáticamente. Sigue tu puesto y el de tus compañeros.', icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg> },
  { title: 'Apuestas por áreas', desc: 'Compite representando a tu sector. El puntaje individual suma al total de tu área.', icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg> },
  { title: 'Historial de pronósticos', desc: 'Revisa todos tus aciertos y errores. Sigue tu progreso durante todo el torneo.', icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg> },
        ...
        <div className="grid lg:grid-cols-2 gap-10 items-end mb-14">
          <div>
            ...
            <h2 className="font-display text-white" style={{ fontSize: 'clamp(2.4rem,6vw,4rem)', lineHeight: 1, letterSpacing: '.01em' }}>
              UNA PLATAFORMA<br /><span style={{ color: '#FF7D00' }}>COMPLETA</span>
            </h2>
          </div>
          <p className="font-body text-base leading-relaxed" style={{ color: 'rgba(255,255,255,.78)' }}>
            Todo lo que necesitas para participar, seguir el torneo y competir con tu equipo está en un solo lugar.
          </p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {FEATURES.map(({ title, desc, icon }) => (
            <div key={title} className="rounded-xl p-5 transition-all duration-300"
              style={{ background: 'linear-gradient(155deg,rgba(88,88,87,.06),rgba(70,0,155,.04))', border: '1px solid rgba(255,125,0,.12)' }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = '#FF7D00'; e.currentTarget.style.transform = 'translateY(-3px)'; e.currentTarget.style.boxShadow = '0 20px 48px rgba(70,0,155,.12)' }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(255,125,0,.12)'; e.currentTarget.style.transform = ''; e.currentTarget.style.boxShadow = '' }}
            >
              ...
              <p className="font-body text-xs leading-relaxed" style={{ color: 'rgba(255,255,255,.72)' }}>{desc}</p>
            </div>
          ))}
        </div>
