...
export default function HomeFeatures() {
  return (
    ...
    <section id="funcionalidades" className="relative" style={{ background: '#0A0A0A', color: '#fff', paddingTop: '5.5rem', paddingBottom: '6rem' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 1.5rem' }}>
        ...
        <div className="grid lg:grid-cols-2 gap-10 items-end mb-14">
          <div>
            ...
            <h2 className="font-display" style={{ fontSize: 'clamp(2.4rem,6vw,4rem)', lineHeight: 1, letterSpacing: 0, color: '#fff' }}>
              UNA PLATAFORMA<br /><span style={{ color: '#E30613' }}>COMPLETA</span>
            </h2>
          </div>
          <p className="font-body text-base leading-relaxed" style={{ color: '#BFBFBF' }}>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {FEATURES.map(({ title, desc, icon }) => (
            <div key={title} className="rounded-xl p-5 transition-all duration-300"
              style={{ background: 'linear-gradient(155deg,rgba(255,255,255,.10),rgba(255,255,255,.035))', border: '1px solid rgba(227,6,19,.18)', color: '#fff' }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = '#E30613'; e.currentTarget.style.transform = 'translateY(-3px)'; e.currentTarget.style.boxShadow = '0 20px 48px rgba(0,0,0,.4)' }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(227,6,19,.18)'; e.currentTarget.style.transform = ''; e.currentTarget.style.boxShadow = '' }}
            >
              <div className="w-11 h-11 rounded-xl flex items-center justify-center mb-4"
                style={{ background: 'rgba(255,255,255,.08)', color: '#E30613', border: '1px solid rgba(255,255,255,.08)' }}>
                {icon}
              </div>
              <h3 className="font-body font-semibold text-sm mb-2" style={{ color: '#fff' }}>{title}</h3>
              <p className="font-body text-xs leading-relaxed" style={{ color: 'rgba(255,255,255,.70)' }}>{desc}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-12 text-center">
          <Link to="/register"
            ...
            style={{ background: '#E30613', color: '#fff', boxShadow: '0 8px 24px rgba(227,6,19,.25)', textDecoration: 'none' }}
      ...
      {/* ── WAVE: features (#0A0A0A) → FAQ (#F7F7F7)
          rect = navy (esta sección), path fill = cream (siguiente sección)
          ESTE ERA EL PROBLEMA: sin rect, el área transparente mostraba #0A0A0A del wrapper */}
      <svg className="absolute bottom-0 left-0 w-full" style={{ display: 'block', height: 80, marginBottom: -2 }}
        ...
        <rect width="1440" height="80" fill="#0A0A0A" />
        <path d="M0,30 C320,80 640,0 960,38 C1120,55 1320,22 1440,32 L1440,80 L0,80 Z" fill="#F7F7F7" />
      </svg>
    </section>
  )
}
