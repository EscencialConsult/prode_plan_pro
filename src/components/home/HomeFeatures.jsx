      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 1.5rem' }}>
        ...
        <div className="grid lg:grid-cols-2 gap-10 items-end mb-14">
          <div>
            <span className="inline-flex items-center gap-2 font-body font-bold text-xs uppercase tracking-widest px-4 py-1.5 rounded-full mb-5"
              style={{ border: '1px solid #0057B8', color: '#0057B8', background: 'rgba(255,255,255,.75)' }}>
              Todo lo que incluye
            </span>
            <h2 className="font-display text-white" style={{ fontSize: 'clamp(2.4rem,6vw,4rem)', lineHeight: 1, letterSpacing: '.01em' }}>
              UNA PLATAFORMA<br /><span style={{ color: '#0057B8' }}>COMPLETA</span>
            </h2>
          </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {FEATURES.map(({ title, desc, icon }) => (
            <div key={title} className="rounded-xl p-5 transition-all duration-300"
              ...
              onMouseEnter={e => { e.currentTarget.style.borderColor = '#0057B8'; e.currentTarget.style.transform = 'translateY(-3px)'; e.currentTarget.style.boxShadow = '0 20px 48px rgba(0,0,0,.4)' }}
        ...
        <div className="mt-12 text-center">
          <Link to="/register"
            ...
            style={{ background: '#0057B8', color: '#05090f', boxShadow: '0 8px 24px rgba(255,255,255,.25)', textDecoration: 'none' }}
