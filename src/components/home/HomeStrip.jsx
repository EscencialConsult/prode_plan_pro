const ITEMS = [
  {
    bg: 'rgba(10,10,10,.12)', stroke: '#333333',
  ...
  {
    bg: 'rgba(227,6,19,.16)', stroke: '#A8030C',
...
export default function HomeStrip() {
  return (
    <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
            {ITEMS.map(({ bg, stroke, title, desc, icon }) => (
              <div key={title} className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="font-body font-semibold text-base mb-1" style={{ color: '#0A0A0A' }}>{title}</h3>
                  <p className="font-body text-sm leading-snug" style={{ color: '#555555' }}>{desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Marquee */}
      <div className="marquee-wrap-el" style={{ background: '#E6E6E6', borderTop: '1px solid rgba(227,6,19,.3)', borderBottom: '1px solid rgba(227,6,19,.3)', padding: '.9rem 0' }}>
        <div className="marquee-track-el font-body font-bold text-xs uppercase tracking-widest" style={{ color: '#0A0A0A' }}>
          {[...MARQUEE, ...MARQUEE].map((item, i) => (
            <span key={i} className="flex items-center gap-2.5 whitespace-nowrap">
              <span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: '#E30613' }} />
              {item}
            </span>
          ))}
        </div>
      ...
      {/* ── WAVE: strip (#F7F7F7) → how-it-works (#F7F7F7 same, wave goes to #0A0A0A later)
