...
export default function HomeFooter() {
  return (
    <footer style={{ background: '#0A0A0A', color: '#fff', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position:'absolute', bottom:0, left:0, width:400, height:250, background:'radial-gradient(ellipse at 0% 100%,rgba(227,6,19,.12),transparent 65%)', pointerEvents:'none' }}/>
      ...
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'3.5rem 1.5rem 2.5rem' }}>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))', gap:'2.5rem' }}>
          ...
          {/* Col 1: Brand */}
          <div>
            {/* Logos Bercovich + Escencial */}
            <div style={{ display:'flex', alignItems:'center', gap:'1rem', marginBottom:'1.25rem' }}>
              <Logo size={34} alt={BRAND.logoAlt} tone="white" style={{ filter:'drop-shadow(0 2px 16px rgba(227,6,19,.3))', opacity:.96 }} />
              <div style={{ width:1, height:36, background:'rgba(227,6,19,.22)', flexShrink:0 }}/>
            ...
            <div style={{ display:'flex', gap:'.6rem' }}>
              <a href="https://wa.me/5491133588062" target="_blank" rel="noopener noreferrer"
                ...
                onMouseEnter={e=>{ e.currentTarget.style.background='rgba(227,6,19,.12)'; e.currentTarget.style.borderColor='rgba(227,6,19,.3)' }}
          ...
          {/* Col 2: Servicios */}
          <div>
            <p style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'.65rem', fontWeight:700, textTransform:'uppercase', letterSpacing:'.2em', color:'rgba(227,6,19,.7)', margin:'0 0 1.1rem' }}>
              Servicios
            </p>
              ...
              {[
                ...
                <li key={label}>
                  <span style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'.84rem', color:'rgba(255,255,255,.45)', display:'inline-flex', alignItems:'center', gap:'.4rem' }}>
                    <span style={{ width:4, height:4, borderRadius:'50%', background:'rgba(227,6,19,.3)', flexShrink:0 }}/>
                    {label}
                  </span>
                </li>
              ))}
      ...
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'0 1.5rem' }}>
        <div style={{ height:1, background:'linear-gradient(90deg,transparent,rgba(227,6,19,.2) 20%,rgba(227,6,19,.2) 80%,transparent)' }}/>
      </div>
      ...
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'1.25rem 1.5rem' }}>
        <div style={{ display:'flex', flexWrap:'wrap', alignItems:'center', justifyContent:'space-between', gap:'.75rem' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:'.85rem' }}>
            <Logo size={22} alt="" tone="white" style={{ opacity:.25 }} />
            <span style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'.72rem', color:'rgba(255,255,255,.25)' }}>
              © 2026 Bercovich · Escencial. Todos los derechos reservados.
            </span>
          </div>
