...
export default function PredictModal({ bet, predictions = {}, onSubmit, onClose, loading }) {
  ...
  function handleSubmit(e) {
    ...
    for (const match of (bet.partidos || [])) {
      // No enviar partidos que ya arrancaron: la base los rechaza (cierre por partido).
      const kickoff = match.fecha_partido ? new Date(match.fecha_partido) : null
      const yaComenzo = kickoff && !isNaN(kickoff.getTime()) && kickoff.getTime() <= Date.now()
      if (yaComenzo) continue
        ...
        <div className="flex flex-col bg-white min-w-0 h-full overflow-hidden">
          ...
          >
            ...
            {bet.partidos?.map((match, idx) => {
              ...
              // Cierre POR PARTIDO: se puede pronosticar hasta que arranca.
              const kickoff = match.fecha_partido ? new Date(match.fecha_partido) : null
              ...
              const fmtKick = kickoff && !isNaN(kickoff.getTime())
                ...
                : null
              const isDisabled = !open || isLive || isFinished || estaBloqueado || matchStarted || esTBD
              ...
              return (
                ...
                >
                  ...
                  {/* Disponibilidad por partido (se bloquea solo al arrancar) */}
                  {open && !estaBloqueado && (
                    (matchStarted || isLive || isFinished) ? (
                      <div className="flex items-center gap-1.5 px-3 py-2 bg-slate-50 border-t border-slate-200 text-[10px] font-bold uppercase tracking-wider text-slate-500">
                      ...
                      <div className="flex items-center gap-1.5 px-3 py-2 bg-green-50 border-t border-green-100 text-[10px] font-bold uppercase tracking-wider text-green-700">
