    ...
    <div className="animate-fade-in delay-2">
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-2">
          <h2 className="font-display text-2xl" style={{ color: '#111811', letterSpacing: '.02em' }}>PENDIENTES</h2>
      ...
      {loadingUsers && pendingUsers.length === 0 && (
        <div className="text-center py-16">
          ...
          <p className="font-body text-sm mt-3" style={{ color: '#4a6b50' }}>Cargando usuarios...</p>
        </div>
      )}
      ...
      {!loadingUsers && pendingUsers.length === 0 && (
        <div className="rounded-2xl p-12 text-center" style={{ background: '#fff', border: '1px dashed #b8c0cc' }}>
          ...
          <p className="font-body font-semibold text-sm" style={{ color: '#4a6b50' }}>No hay usuarios pendientes</p>
          <p className="font-body text-xs mt-1" style={{ color: '#8aaa8e' }}>Las solicitudes nuevas van a aparecer acá.</p>
        </div>
      )}
          ...
          {pendingUsers.map(u => {
            ...
            return (
              <div key={u.id} className="rounded-2xl p-5 transition-all"
                style={{
                  background: '#fff',
                  border: `1px solid ${isApproving ? '#86C873' : '#e2eede'}`,
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    <div className="flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center font-display text-lg"
                      style={{ background: 'linear-gradient(135deg,#111811,#263328)', color: '#86C873', boxShadow: '0 4px 12px rgba(17,24,17,.2)' }}>
                ...
                {/* Panel aprobación */}
                {isApproving && (
                  <div className="mt-5 pt-5 flex flex-col gap-5" style={{ borderTop: '1px solid #e2eede' }}>

                    {/* Rol */}
                    <div>
                      <p className="font-body font-bold text-xs uppercase tracking-widest mb-2" style={{ color: '#4a6b50' }}>Rol</p>
                        ...
                        {[
                          ...
                          return (
                            <button key={opt.val} type="button"
                              ...
                              style={{
                                background: isActive ? '#111811' : '#f0f5ee',
                                border: `1px solid ${isActive ? '#111811' : '#b8c0cc'}`,
                                color: isActive ? '#86C873' : '#111811',
                              }}>
                              <p className="font-body font-semibold text-sm">{opt.label}</p>
                              <p className="font-body text-xs mt-0.5" style={{ color: isActive ? 'rgba(134,200,115,.6)' : '#4a6b50' }}>{opt.desc}</p>
                            </button>
                          )
                        })}
                      </div>
                    ...
                    {/* Área */}
                    <div>
                      <p className="font-body font-bold text-xs uppercase tracking-widest mb-2" style={{ color: '#4a6b50' }}>Área</p>
                      {areas.length === 0 ? (
                        <p className="text-xs font-body p-3 rounded-xl"
                          style={{ background: 'rgba(134,200,115,.06)', border: '1px solid rgba(134,200,115,.2)', color: '#5A9E4A' }}>
                          ...
                          {areas.map(a => {
                            ...
                            return (
                              <button key={a.id} type="button"
                                ...
                                style={{
                                  background: isActive ? '#111811' : 'transparent',
                                  border: `1px solid ${isActive ? '#111811' : '#b8c0cc'}`,
                                  color: isActive ? '#86C873' : '#4a6b50',
                                }}>
                                {a.nombre}
                              </button>
                            )
