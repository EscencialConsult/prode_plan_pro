...
function BtnGold({ children, onClick, disabled }) {
  return (
    <button
      ...
      style={{ background: '#0c182b', color: '#7dd3fc', boxShadow: '0 4px 14px rgba(12,24,43,.2)' }}
    ...
    <div className="animate-fade-in delay-2">
      ...
      {loadingUsers && pendingUsers.length === 0 && (
        <div className="text-center py-16">
          <span className="inline-block w-8 h-8 rounded-full border-2 border-t-transparent animate-spin" style={{ borderColor: '#7dd3fc' }} />
          ...
          {pendingUsers.map(u => {
            ...
            return (
              <div key={u.id} className="rounded-2xl p-5 transition-all"
                style={{
                  background: '#fff',
                  border: `1px solid ${isApproving ? '#7dd3fc' : '#f0eadb'}`,
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    <div className="flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center font-display text-lg"
                      style={{ background: 'linear-gradient(135deg,#0c182b,#17376a)', color: '#7dd3fc', boxShadow: '0 4px 12px rgba(12,24,43,.2)' }}>
                          ...
                          return (
                            <button key={opt.val} type="button"
                              ...
                              style={{
                                ...
                                color: isActive ? '#7dd3fc' : '#0c182b',
                              }}>
                              <p className="font-body font-semibold text-sm">{opt.label}</p>
                              <p className="font-body text-xs mt-0.5" style={{ color: isActive ? 'rgba(125,211,252,.6)' : '#5f6e8a' }}>{opt.desc}</p>
                            </button>
                          )
                        })}
                      </div>
                    ...
                    <div>
                      ...
                      {areas.length === 0 ? (
                        <p className="text-xs font-body p-3 rounded-xl"
                          style={{ background: 'rgba(125,211,252,.06)', border: '1px solid rgba(125,211,252,.2)', color: '#0ea5e9' }}>
