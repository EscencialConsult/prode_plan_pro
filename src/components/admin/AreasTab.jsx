...
function BtnGold({ children, onClick, disabled, type = 'button' }) {
  return (
    <button
      ...
      style={{ background: '#0c182b', color: '#E30613', boxShadow: '0 4px 14px rgba(12,24,43,.2)' }}
      ...
      >
        <div className="flex items-center gap-3 mb-6">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center"
            style={{ background: 'rgba(227,6,19,.12)', border: '1px solid rgba(227,6,19,.25)' }}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#A8030C" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
        ...
        <form onSubmit={handleCreateArea} className="flex flex-col gap-4">
          ...
          <BtnGold type="submit" disabled={savingArea || !newArea.nombre.trim()} onClick={undefined}>
            {savingArea
              ? <><span className="w-4 h-4 rounded-full border-2 border-t-transparent inline-block animate-spin" style={{ borderColor: '#E30613' }} /> Creando...</>
              : 'Crear área'
            }
          </BtnGold>
        </form>
      ...
      <div className="lg:col-span-3 flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-display text-xl" style={{ color: '#E30613' }}>({areasAll.length})</span>
          </div>
        ...
        {loadingAreas && areasAll.length === 0 && (
          <div className="text-center py-16">
            <span className="inline-block w-8 h-8 rounded-full border-2 border-t-transparent animate-spin" style={{ borderColor: '#E30613' }} />
