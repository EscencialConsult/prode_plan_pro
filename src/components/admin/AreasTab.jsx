    ...
    <div className="animate-fade-in delay-2 grid lg:grid-cols-5 gap-6">
      ...
      >
        <div className="flex items-center gap-3 mb-6">
          ...
          <h2 className="font-display text-2xl" style={{ color: '#1f1f23', letterSpacing: '.02em' }}>NUEVA ÁREA</h2>
        </div>

        <form onSubmit={handleCreateArea} className="flex flex-col gap-4">
          <div>
            <p className="font-body font-bold text-xs uppercase tracking-widest mb-2" style={{ color: '#6e6f73' }}>Nombre *</p>
          ...
          <div>
            <p className="font-body font-bold text-xs uppercase tracking-widest mb-2" style={{ color: '#6e6f73' }}>Descripción (opcional)</p>
          ...
          <BtnGold type="submit" disabled={savingArea || !newArea.nombre.trim()} onClick={undefined}>
            {savingArea
              ? <><span className="w-4 h-4 rounded-full border-2 border-t-transparent inline-block animate-spin" style={{ borderColor: '#ffffff' }} /> Creando...</>
              : 'Crear área'
            }
          </BtnGold>
        </form>
      ...
      <div className="lg:col-span-3 flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <h2 className="font-display text-2xl" style={{ color: '#1f1f23', letterSpacing: '.02em' }}>ÁREAS</h2>
            <span className="font-display text-xl" style={{ color: '#c02727' }}>({areasAll.length})</span>
          </div>
        ...
        {loadingAreas && areasAll.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="font-body text-sm mt-3" style={{ color: '#6e6f73' }}>Cargando áreas...</p>
          </div>
        )}
        ...
        {!loadingAreas && areasAll.length === 0 && (
          <div className="rounded-2xl p-10 text-center" style={{ background: '#fff', border: '1px dashed #e8dfd0' }}>
            ...
            <p className="font-body text-sm font-semibold" style={{ color: '#6e6f73' }}>Todavía no hay áreas creadas.</p>
            <p className="font-body text-xs mt-1" style={{ color: '#a9a9ae' }}>Creá la primera desde el formulario.</p>
          </div>
        )}
            ...
            {areasAll.map(area => {
              ...
              return (
                <div key={area.id} className="rounded-xl p-4 transition-all"
                  style={{
                    background: '#fff',
                    border: `1px solid ${isActive ? '#faecec' : '#c9cacd'}`,
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          <p className="font-display text-lg truncate" style={{ color: '#1f1f23', letterSpacing: '.01em' }}>{area.nombre}</p>
                          <span className="inline-flex px-2.5 py-0.5 rounded-full text-xs font-body font-bold uppercase tracking-wider"
                            style={{
                              ...
                              color:      isActive ? '#1b8a5a'              : '#a9a9ae',
                              border:     `1px solid ${isActive ? 'rgba(27,138,90,.2)' : 'rgba(95,110,138,.15)'}`,
                            }}>
                            {isActive ? 'Activa' : 'Inactiva'}
                          </span>
                        </div>
                        {area.descripcion && <p className="text-xs font-body mt-1" style={{ color: '#6e6f73' }}>{area.descripcion}</p>}
                      </div>
