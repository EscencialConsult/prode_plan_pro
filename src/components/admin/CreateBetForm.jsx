  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <input
        ...
        onFocus={e => {
          e.target.style.borderColor = error ? '#e03252' : '#A6C934'
          e.target.style.boxShadow = error 
            ? '0 0 0 3px rgba(224,50,82,.08)' 
            : '0 0 0 3px rgba(166,201,52,.08)'
        }}
        onBlur={e => {
  ...
  return (
    <button
      ...
      style={{
        ...
        color: active ? '#A6C934' : '#5f6e8a',
      }}
      onMouseEnter={e => { 
        if (!active && !disabled) { 
          e.currentTarget.style.borderColor = '#A6C934'
          e.currentTarget.style.color = '#8fae27'
        } 
      }}
      onMouseLeave={e => { 
        if (!active && !disabled) { 
...
export default function CreateBetForm({ onSubmit, loading, matches = [] }) {
  ...
  useEffect(() => {
    ...
    // Autocompletado: si no hay cierre global, lo sugerimos = inicio del último
    // partido. Así la apuesta cubre toda la fase y cada partido se bloquea solo
    // a su propia hora de inicio. El admin puede ajustarlo después.
  ...
  // Sin tope superior: el cierre global puede ser cualquier momento posterior
  // al último partido (el bloqueo real es por partido individual).
  const maxFechaCierre = ''

  // El cierre global no puede ser anterior al inicio del último partido.
  const minFechaCierre = useMemo(() => {
    const base = ultimoPartido
      ? new Date(ultimoPartido.fecha_partido)
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          ...
          <span className="font-body text-xs" style={{ color: '#a8b2c4' }}>
            <span className="font-bold" style={{ color: seleccionados > 0 ? '#8fae27' : '#a8b2c4' }}>
              {seleccionados}
            </span>
        ...
        <input
          ...
          onFocus={e => { 
            ...
            e.target.style.boxShadow = '0 0 0 3px rgba(166,201,52,.08)' 
          }}
          onBlur={e => { 
        ...
        <div className="flex items-center justify-between gap-2 text-xs font-body">
          <button type="button" onClick={toggleVisibles} disabled={partidosFiltrados.length === 0}
            className="transition-colors disabled:opacity-40 font-semibold" style={{ color: '#8fae27' }}
            onMouseEnter={e => { if (partidosFiltrados.length > 0) e.currentTarget.style.color = '#A6C934' }}
            onMouseLeave={e => { e.currentTarget.style.color = '#8fae27' }}>
              ...
              return (
                <div key={`${gr.fase}-${gr.jornada}-${gr.grupo}`}>
                  ...
                  <div className="sticky top-0 z-10 px-4 py-2.5 flex items-center justify-between"
                    style={{ background: 'linear-gradient(135deg, #0a1226 0%, #1a2540 100%)', borderBottom: '1px solid rgba(166,201,52,0.15)' }}>
                    <div className="flex items-center gap-2">
                      <span className="w-6 h-6 rounded-md flex items-center justify-center font-display font-bold text-xs"
                        style={{ background: '#A6C934', color: '#0a1226' }}>
                        {gr.partidos.length}
                      </span>
                  ...
                  {gr.partidos.map(m => {
                    ...
                    return (
                      <label
                        ...
                        style={{
                          background: checked ? 'rgba(166,201,52,0.08)' : '#fff',
                          borderBottom: '1px solid #f5f5f5',
                          opacity: noDisponible ? 0.5 : 1,
                          cursor: noDisponible ? 'not-allowed' : 'pointer',
                        }}
                        onMouseEnter={e => { 
                          if (!checked && !noDisponible) e.currentTarget.style.background = 'rgba(166,201,52,0.04)' 
                        }}
                        onMouseLeave={e => { 
                      ...
                      >
                        ...
                        <span className="flex-shrink-0 w-5 h-5 rounded flex items-center justify-center transition-all"
                          style={{
                            border: `2px solid ${checked ? '#A6C934' : '#e8dfd0'}`,
                            background: checked ? '#A6C934' : '#fff',
                          }}>
                          {checked && (
      ...
      {isPro && form.type === 'grupos' && (
        <div className="flex items-start gap-3 p-4 rounded-xl"
          style={{ border: '1.5px solid rgba(166,201,52,.2)', background: 'rgba(166,201,52,.04)' }}>
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#8fae27" strokeWidth="2"
          ...
          <div style={{ flex: 1 }}>
            <p className="font-body font-bold text-xs uppercase tracking-widest mb-1" style={{ color: '#8fae27' }}>
              Apuesta por áreas
            </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="flex flex-col gap-1.5">
          <Field
            label={ultimoPartido
              ? `Cierre global (después del ${fmtFecha(ultimoPartido.fecha_partido)})`
              : 'Cierre global (seleccioná partidos primero)'}
            type="datetime-local"
          ...
          {ultimoPartido && !errorFecha && (
            <div className="flex items-start gap-2 px-3 py-2 rounded-lg"
              style={{ background: 'rgba(166,201,52,0.06)', border: '1px solid rgba(166,201,52,0.15)' }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#8fae27" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="flex-shrink-0 mt-0.5">
                <circle cx="12" cy="12" r="10"/>
                <line x1="12" y1="16" x2="12" y2="12"/>
                <line x1="12" y1="8" x2="12.01" y2="8"/>
              </svg>
              <span className="font-body text-[11px]" style={{ color: '#5f6e8a' }}>
                Cada partido se bloquea automáticamente al comenzar. La apuesta queda abierta hasta el <strong style={{ color: '#0a1226' }}>{fmtFecha(ultimoPartido.fecha_partido)}</strong> (último partido: {ultimoPartido.equipo_local} vs {ultimoPartido.equipo_visitante}).
              </span>
            </div>
          )}
        </div>
