  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <input
        ...
        onFocus={e => {
          e.target.style.borderColor = error ? '#e03252' : '#7dd3fc'
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          ...
          <span className="font-body text-xs" style={{ color: '#a8b2c4' }}>
            <span className="font-bold" style={{ color: seleccionados > 0 ? '#0ea5e9' : '#a8b2c4' }}>
              {seleccionados}
            </span>
        ...
        <input
          ...
          onFocus={e => { 
            ...
            e.target.style.boxShadow = '0 0 0 3px rgba(125,211,252,.08)' 
          }}
          onBlur={e => { 
        ...
        <div className="flex items-center justify-between gap-2 text-xs font-body">
          <button type="button" onClick={toggleVisibles} disabled={partidosFiltrados.length === 0}
            className="transition-colors disabled:opacity-40 font-semibold" style={{ color: '#0ea5e9' }}
            onMouseEnter={e => { if (partidosFiltrados.length > 0) e.currentTarget.style.color = '#7dd3fc' }}
              ...
              return (
                <div key={`${gr.fase}-${gr.jornada}-${gr.grupo}`}>
                  ...
                  <div className="sticky top-0 z-10 px-4 py-2.5 flex items-center justify-between"
                    style={{ background: 'linear-gradient(135deg, #0a1226 0%, #1a2540 100%)', borderBottom: '1px solid rgba(125,211,252,0.15)' }}>
                  ...
                  {gr.partidos.map(m => {
                    ...
                    return (
                      <label
                        ...
                        style={{
                          background: checked ? 'rgba(125,211,252,0.08)' : '#fff',
                          borderBottom: '1px solid #f5f5f5',
                          opacity: noDisponible ? 0.5 : 1,
                          cursor: noDisponible ? 'not-allowed' : 'pointer',
                        }}
                        onMouseEnter={e => { 
                          if (!checked && !noDisponible) e.currentTarget.style.background = 'rgba(125,211,252,0.04)' 
                        }}
                        onMouseLeave={e => { 
                      ...
                      >
                        ...
                        <span className="flex-shrink-0 w-5 h-5 rounded flex items-center justify-center transition-all"
                          style={{
                            border: `2px solid ${checked ? '#7dd3fc' : '#e8dfd0'}`,
                            background: checked ? '#7dd3fc' : '#fff',
                          }}>
                          {checked && (
      ...
      {isPro && form.type === 'grupos' && (
        <div className="flex items-start gap-3 p-4 rounded-xl"
          style={{ border: '1.5px solid rgba(125,211,252,.2)', background: 'rgba(125,211,252,.04)' }}>
          ...
          <div style={{ flex: 1 }}>
            <p className="font-body font-bold text-xs uppercase tracking-widest mb-1" style={{ color: '#0ea5e9' }}>
              Apuesta por áreas
            </p>
