export default function AdminHeader({ bets = [], pendingUsers = [] }) {
  ...
  const stats = [
    ...
    { label:'Finalizadas',    value:finishedBets,        color:'#9e1f1f', accent:'rgba(192,39,39,.07)',border:'rgba(192,39,39,.2)',  icon:<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg> },
    { label:'Pendientes',     value:pendingUsers.length, color: pendingUsers.length > 0 ? '#e03252' : '#6e6f73', accent: pendingUsers.length > 0 ? 'rgba(224,50,82,.07)' : 'rgba(110,111,115,.05)', border: pendingUsers.length > 0 ? 'rgba(224,50,82,.2)' : 'rgba(110,111,115,.12)', icon:<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg> },
  ]
...
<h1 style={{ 
  ...
  PANEL DE <span style={{ color:'#c02727' }}>CONFIGURACIÓN</span>
</h1>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(170px,1fr))', gap:'.75rem' }}>
        {stats.map(({ label, value, color, accent, border, icon }) => (
          <div key={label}
            style={{ background:'#fff', border:`1px solid ${border}`, borderRadius:14, padding:'1rem 1.2rem', display:'flex', alignItems:'center', gap:'.85rem', boxShadow:'0 1px 0 rgba(31,31,35,.04)', transition:'transform .18s,box-shadow .18s', cursor:'default' }}
            ...
            <div>
              ...
              <p style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'.63rem', fontWeight:700, textTransform:'uppercase', letterSpacing:'.1em', color:'#a9a9ae', margin:'.2rem 0 0' }}>{label}</p>
            </div>
          </div>
        ))}
      </div>
